...
class PostgresArrayToPHPArrayTransformerTest extends TestCase
{
    ...
    public function test_text_array_column_storage(): void
    {
        ...
        foreach ($testData as $id => $arrayData) {
            ...
            self::assertEquals(
                $arrayData, 
                $retrieved, 
                "Round-trip failed for test data ID {$id}: " . var_export($arrayData, true)
            );
        }
    }
    ...
    private function assertRoundTripIntegrity(array $originalArray, string $testDescription): void
    {
        ...
        if (empty($originalArray)) {
            ...
            $sql = "SELECT ARRAY[{$placeholders}]::text[]::text";
            $stmt = $this->connection->prepare($sql);
        ...
        self::assertEquals(
            $originalArray,
            $parsed,
            "Round-trip integrity failed for {$testDescription}. PostgreSQL representation: {$postgresString}"
        );
    }
    ...
    private function insertArrayData(int $id, array $arrayData): void
    {
        ...
        $stmt->bindValue('arrayData', $arrayData, 'text[]');
