<?php

declare(strict_types=1);

namespace Tests\Integration\MartinGeorgiev\Doctrine\ORM\Query\AST\Functions;

use MartinGeorgiev\Doctrine\ORM\Query\AST\Functions\Crc32;
use PHPUnit\Framework\Attributes\Test;

final class Crc32Test extends TextTestCase
{
    protected function setUp(): void
    {
        parent::setUp();
        $this->requirePostgresVersion(180000, 'crc32 function');
    }

    protected function getStringFunctions(): array
    {
        return [
            'CRC32' => Crc32::class,
        ];
    }

    #[Test]
    public function returns_the_crc32_checksum_from_a_text_literal(): void
    {
        $dql = "SELECT CRC32('Hello Doctrine') as result
                FROM Fixtures\\MartinGeorgiev\\Doctrine\\Entity\\ContainsTexts t
                WHERE t.id = 1";

        $result = $this->executeDqlQuery($dql);
        $this->assertIsInt($result[0]['result']);
        $this->assertSame(2004976067, $result[0]['result']);
    }

    #[Test]
    public function returns_the_crc32_checksum_from_an_entity_field(): void
    {
        $dql = 'SELECT CRC32(t.text1) as result
                FROM Fixtures\\MartinGeorgiev\\Doctrine\\Entity\\ContainsTexts t
                WHERE t.id = 1';

        $result = $this->executeDqlQuery($dql);
        $this->assertIsInt($result[0]['result']);
        $this->assertSame(4236695221, $result[0]['result']);
    }
}
