...
class ST_SimplifyVWTest extends SpatialOperatorTestCase
{
    ...
    {
        $dql = 'SELECT ST_EQUALS(ST_SIMPLIFYVW(g.geometry1, 0.1), g.geometry1) as result
                FROM Fixtures\MartinGeorgiev\Doctrine\Entity\ContainsGeometries g
                WHERE g.id = 1';
    ...
    {
        $dql = 'SELECT ST_LENGTH(ST_SIMPLIFYVW(g.geometry1, :tolerance)) as result
                FROM Fixtures\MartinGeorgiev\Doctrine\Entity\ContainsGeometries g
                WHERE g.id = 3';
    ...
    {
        $dql = 'SELECT ST_LENGTH(ST_SIMPLIFYVW(g.geometry1, ABS(0.1))) as result
                FROM Fixtures\MartinGeorgiev\Doctrine\Entity\ContainsGeometries g
                WHERE g.id = 3';
