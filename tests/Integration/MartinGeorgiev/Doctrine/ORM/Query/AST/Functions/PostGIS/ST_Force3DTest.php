...
final class ST_Force3DTest extends SpatialOperatorTestCase
{
    ...
    #[Test]
    public function returns_the_3d_form_of_a_wkt_literal(): void
    {
        $dql = "SELECT ST_COORDDIM(ST_FORCE3D(ST_GEOMFROMTEXT('POINT(1 2)'))) as result
    ...
    #[Test]
    public function returns_the_3d_form_of_an_entity_field(): void
    {
        $dql = 'SELECT ST_COORDDIM(ST_FORCE3D(g.geometry1)) as result
                FROM Fixtures\MartinGeorgiev\Doctrine\Entity\ContainsGeometries g
                WHERE g.id = 1';
