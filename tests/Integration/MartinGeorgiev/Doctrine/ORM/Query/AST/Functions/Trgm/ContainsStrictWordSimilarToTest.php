...
final class ContainsStrictWordSimilarToTest extends TestCase
{
    ...
    #[Test]
    public function returns_true_when_haystack_entity_field_contains_needle_as_whole_word(): void
    {
        $dql = "SELECT t.id FROM Fixtures\\MartinGeorgiev\\Doctrine\\Entity\\ContainsTexts t
                WHERE CONTAINS_STRICT_WORD_SIMILAR_TO(t.text1, 'test') = TRUE AND t.id = 1";
    ...
    {
        $dql = 'SELECT t.id FROM Fixtures\MartinGeorgiev\Doctrine\Entity\ContainsTexts t
                WHERE CONTAINS_STRICT_WORD_SIMILAR_TO(t.text1, t.text2) = TRUE AND t.id = 3';
        $result = $this->executeDqlQuery($dql);
