...
final class ST_Force4DTest extends SpatialOperatorTestCase
{
    ...
    #[Test]
    public function returns_the_4d_form_of_a_wkt_literal(): void
    {
        $dql = "SELECT ST_COORDDIM(ST_FORCE4D(ST_GEOMFROMTEXT('POINT(1 2)'))) as result
    ...
    #[Test]
    public function returns_the_4d_form_of_an_entity_field(): void
    {
        $dql = 'SELECT ST_COORDDIM(ST_FORCE4D(g.geometry1)) as result
                FROM Fixtures\MartinGeorgiev\Doctrine\Entity\ContainsGeometries g
                WHERE g.id = 11';
