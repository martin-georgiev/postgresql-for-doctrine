...
{
    protected function createFixture(): JsonbPathQuery
    {
    ...
    {
        $this->expectException(InvalidBooleanException::class);
        $this->expectExceptionMessage('The boolean parameter for jsonb_path_query must be a string literal');

        $dql = \sprintf("SELECT JSONB_PATH_QUERY(e.jsonbObject1, '$.items[*].id', '{\"strict\": false}', e.jsonbObject1) FROM %s e", ContainsJsons::class);
        $this->buildEntityManager()->createQuery($dql)->getSQL();
    }
