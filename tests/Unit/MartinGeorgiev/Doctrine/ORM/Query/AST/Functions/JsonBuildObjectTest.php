...
class JsonBuildObjectTest extends BaseVariadicFunctionTestCase
{
    ...
    {
        return [
            'builds JSON object with field value' => "SELECT json_build_object('key1', c0_.jsonObject1) AS sclr_0 FROM ContainsJsons c0_",
            ...
            'builds JSON object with multiple field values' => "SELECT json_build_object('key1', c0_.jsonObject1, 'key2', c0_.jsonObject2) AS sclr_0 FROM ContainsJsons c0_",
            ...
            'builds JSON object with mixed field and literal values' => "SELECT json_build_object('field_key', c0_.jsonObject1, 'literal_key', 'literal_value') AS sclr_0 FROM ContainsJsons c0_",
        ];
    }
    ...
    {
        return [
            'builds JSON object with field value' => \sprintf("SELECT JSON_BUILD_OBJECT('key1', e.jsonObject1) FROM %s e", ContainsJsons::class),
            ...
            'builds JSON object with multiple field values' => \sprintf("SELECT JSON_BUILD_OBJECT('key1', e.jsonObject1, 'key2', e.jsonObject2) FROM %s e", ContainsJsons::class),
            ...
            'builds JSON object with mixed field and literal values' => \sprintf("SELECT JSON_BUILD_OBJECT('field_key', e.jsonObject1, 'literal_key', 'literal_value') FROM %s e", ContainsJsons::class),
        ];
    }
    ...
    public function throws_exception_for_odd_number_of_arguments(): void
    {
        ...
        $dql = \sprintf("SELECT JSON_BUILD_OBJECT('key1', e.jsonObject1, 'key2') FROM %s e", ContainsJsons::class);
