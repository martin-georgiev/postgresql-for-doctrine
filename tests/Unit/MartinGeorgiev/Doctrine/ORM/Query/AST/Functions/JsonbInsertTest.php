...
class JsonbInsertTest extends TestCase
{
    ...
    {
        $this->expectException(InvalidBooleanException::class);
        $this->expectExceptionMessage('The boolean parameter for jsonb_insert must be a string literal');

        $dql = \sprintf("SELECT JSONB_INSERT(e.jsonbObject1, '{country}', '{\"iso_3166_a3_code\":\"bgr\"}', e.jsonbObject1) FROM %s e", ContainsJsons::class);
        $this->buildEntityManager()->createQuery($dql)->getSQL();
    }
