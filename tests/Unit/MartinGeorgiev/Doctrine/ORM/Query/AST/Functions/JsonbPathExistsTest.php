...
final class JsonbPathExistsTest extends BaseVariadicFunctionTestCase
{
    protected function createFixture(): JsonbPathExists
    {
    ...
    {
        $this->expectException(InvalidBooleanException::class);
        $this->expectExceptionMessage('The boolean parameter for jsonb_path_exists must be a string literal');

        $dql = \sprintf("SELECT JSONB_PATH_EXISTS(e.jsonbObject1, '$.items[*].id', '{\"strict\": false}', e.jsonbObject1) FROM %s e", ContainsJsons::class);
        $this->buildEntityManager()->createQuery($dql)->getSQL();
    }
