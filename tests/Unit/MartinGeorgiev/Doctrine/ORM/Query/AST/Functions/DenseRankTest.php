...
final class DenseRankTest extends TestCase
{
    ...
    {
        return [
            'without a window specification' => \sprintf('SELECT OVER(DENSE_RANK()) FROM %s e', ContainsNumerics::class),
            'with PARTITION BY' => \sprintf('SELECT OVER(DENSE_RANK(), PARTITION BY e.integer1) FROM %s e', ContainsNumerics::class),
            'with ORDER BY' => \sprintf('SELECT OVER(DENSE_RANK(), ORDER BY e.integer2) FROM %s e', ContainsNumerics::class),
            'with PARTITION BY and ORDER BY' => \sprintf('SELECT OVER(DENSE_RANK(), PARTITION BY e.integer1 ORDER BY e.integer2 DESC) FROM %s e', ContainsNumerics::class),
        ];
    }
    ...
    public function throws_exception_when_argument_is_provided(): void
    {
        ...
        $dql = \sprintf('SELECT OVER(DENSE_RANK(1), ORDER BY e.integer2) FROM %s e', ContainsNumerics::class);
