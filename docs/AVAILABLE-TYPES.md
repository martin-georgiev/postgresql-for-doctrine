...
|---|---|---|
| bytea | bytea | `MartinGeorgiev\Doctrine\DBAL\Types\Bytea` (see [note](#bytea-type)) |
|---|---|---|
...
The `bytea` type maps PostgreSQL's [`bytea`](https://www.postgresql.org/docs/current/datatype-binary.html) binary data type. Values are stored as raw binary and returned as PHP strings.

PostgreSQL returns bytea values in hex format by default (prefixed with `\x`). This type handles decoding transparently — both hex format and legacy escape format are supported on input.
...
- PHP/PDO may return bytea values as stream resources depending on the driver version — this type handles both strings and resources transparently
- Values written to the database must be PHP strings containing raw binary data
- The hex decoding only applies when reading from the database — writes pass binary strings through as-is
