...
- Neither option → bare `GEOMETRY` / `GEOGRAPHY` (unchanged behaviour).
...
An unknown `geometry_type` or a non-integer `srid` throws `InvalidSpatialColumnDeclarationException` while the DDL is generated, rather than producing a column definition PostgreSQL will reject later.
