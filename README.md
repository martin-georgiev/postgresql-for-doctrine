...
- **Aggregate Functions**
  ...
  - Conditional aggregation with `FILTER (WHERE ...)` on any aggregate
  ...
  - Ordered-set aggregates with `WITHIN GROUP (ORDER BY ...)` (`percentile_cont`, `percentile_disc`, `mode`)
...
- **Window Functions**
  ...
  - Any aggregate as a window function (`OVER`), with `ROWS`, `RANGE` and `GROUPS` frame clauses
